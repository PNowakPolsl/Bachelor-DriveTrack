...
export default function Profile() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const backendMsg =
        err?.response?.data ?? "Nie udało się zmienić hasła.";
      setError(backendMsg);
      setTimeout(() => setError(""), 4000);
    } finally {
      setLoading(false);
          ...
          <div className="bg-white rounded-2xl shadow-md p-8 mt-10 max-w-2xl">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="mt-2 bg-blue-600 text-white font-semibold text-lg py-3 rounded-lg hover:bg-blue-500 transition disabled:opacity-60"
              >
                {loading ? "Aktualizuję hasło..." : "Aktualizuj hasło"}
              </button>
